...
const StaffLayouting: React.FC = () => {
	...
	const handleConfirmProceedToFPR = async () => {
		if (selectedRecord && dateFinished && proofreader && proofreaderEmail) {
			...
			await sendMail({
				message: `<p>Hello ${proofreader}, you are assigned to the manuscript entitled ${selectedRecord.title} by ${selectedRecord.authors[0]} as Final Proofreader.</p>`,
				recipients: [proofreaderEmail],
				subject: 'Manuscript Assigned to Proofreader',
			});
