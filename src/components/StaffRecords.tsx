...
const StaffRecords: React.FC = () => {
	...
	const handleUpdateStatus = async () => {
		if (selectedManuscript) {
			...
			await sendMail({
				message: `<p>Hello ${selectedManuscript.authors[0]} <br/> Your manuscript entitled ${selectedManuscript.title} needs revision. <br/>Comment: ${revisionComment}</p>`,
	...
	const confirmRejection = async () => {
		await updateManuscript(selectedManuscript._id, {
			...
			rejectComment: rejectionComment,
			rejectDate: new Date().toISOString(),
		});

		await sendMail({
			message: `<p>Hello ${selectedManuscript.authors[0]} <br/> Your manuscript entitled ${selectedManuscript.title} has been rejected. <br/>Reason: ${rejectionReason}</p> <br/>Additional Comment: ${rejectionComment}`,
			recipients: [selectedManuscript.authorEmail],
						...
						<div className="space-y-4">
							...
							{[...Array(4)].map((_, index) => (
								<div key={index}>
									...
									<select
										title={`selectreviewer${index + 1}`}
										value={selectedReviewers[index] || ''}
										onChange={(e) =>
											setSelectedReviewers((prevState) => {
												const updatedReviewers = [...prevState];
												updatedReviewers[index] = e.target.value;
												return updatedReviewers;
											})
										}
										className="w-full p-2 border rounded focus:ring-2 focus:ring-blue-500"
										required={index < 2} // Required for the first two reviewers
									>
										...
										{reviewers
											.filter(
												(reviewer) =>
													...
													selectedReviewers[index] === reviewer._id
